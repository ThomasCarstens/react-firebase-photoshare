...
const HomeScreen = (props) => {
  ...
  useEffect(() => {
    ...
    if (gameIsThreaded){
      setLearningLevel(spoofMacroGameSets[macroName][macroLevel][3])
    }
  ...
  var gameName = spoofMacroGameSets[macroName][gameSetLevel][0]//spoofGameSets[selectedGame][gameSetLevel]
  ...
  const progressCalculate = () => {
    ...
    if (correctLeftInGallery.length == 0){
      ...
      if (gameComplete && (gameSetLevel >= Object.keys(spoofMacroGameSets[macroName]).length)){
        setGameSetComplete(true)
        // #RECORD_ACCURACY
      } else {
